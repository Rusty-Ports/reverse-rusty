...
and ADR-095 replaced whole-sweep serialization with conflict-free parallel move waves.
[ADR-171](adr-171-cluster-reassign-api-contract.md) added live-primary attestation and safe RF=1
reconciliation. [ADR-175](adr-175-durable-reassignment-intent-and-conditional-cutover.md) supersedes
this ADR's move-then-commit crash window and single-coordinator boundary: native movement now records
replicated intent, conditionally commits before the live swap, and resolves every unfinished phase
before startup serves. Raw handoff remains explicitly uncommitted.
